<?php
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if (!preg_match("/^[A-Za-z][A-Za-z0-9]{5,14}$/", $loginName))
            die("Bad Login Name!");
        $passregex = "/^(?=.*\d)(?=.*[@^_+=[\]:])(?=.*[A-Z])(?=.*[a-z])\S{8,15}$/";
        $password1 = sanitized_input($_POST["password1"]);
        if (!preg_match($passregex, $password1))
    ...
    if(emailAlreadyExists($db, $_POST['email'])) {
        ...
        $loginPassword = md5($_POST['password2']);
        ...
        $query = "INSERT INTO my_Customers (
            ...
            email, phone, street, city, region, postal_code, date_time,
        ...
        if (mysqli_query($db, $query)) {
            echo "<h3>Thank you for registering with Badal Sports Collection.<br>
