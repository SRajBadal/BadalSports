...
if ($numRecords == 0 && $productID == 'view') {
    ...
    if($numRecords == 0) {
        ...
        for ($i=1; $i <= $numRecords; $i++){
            $grandTotal += displayExistingItemsColumns($db, $items);
        }
    }
...
function displayHeader() {
    echo 
        "<form id='orderForm' action='scripts/shopping_cart_addItem.php'>
            <table cellpadding='4px' cellspacing='2px' border='2px' 
...
function displayExistingItemsColumns($db, $items) {
    ...
    echo 
        "<td class='w3-center'>
            ...
            <a class='w3-button w3-green w3-round w3-small'
                style = 'margin-bottom: 3px'
            href='scripts/shopping_cart_deleteItem.php?
