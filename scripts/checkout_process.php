...
function displayReceipt($db, $customerID)
{
    ...
    $numsRecords = mysqli_num_rows($items);
    if ($numsRecords == 0) {
        echo"
            <h4 class='w3-center'><strong>Shopping Cart</strong></h4>
...
function displayReceiptHeader()
{
    ...
    $time = date('g:ia');
    echo "
    <p class='w3-center'><strong>******RECEIPT******</strong></p>
    <h4 calss='w3-center'>
    Payment received from
    $_SESSION[salutation]
    $_SESSION[first_name]
    $_SESSION[middle_initial]
    $_SESSION[last_name] on $date $time.
    </h4>
    ";
    echo"
    <div style='overflow-x:auto;'>
    <table class='w3-table w3-border w3-bordered w3-border-black'>
        <tr>
            <th>Product Image</th>
            <th>Product Name</th>
            <th>Price</th>
...
/**
 * It displays the the items and returns the total
 * price of the items
 */
...
function displayReceiptFooter($grandTotal)
{
    ...
    echo "
            ...
            <td colspan='5'>
                ...
                <br>Thank you very much for shopping with Dhakatopi.
                <br>We appreciate your purchase of the above product
                <br>You may print a copy of this page for your permanent
                record.
...
function markOrderItemsPaid($db, $orderID)
{
    ...
    if ($orderItems != NULL) {
        $numsRecords = mysqli_num_rows($orderItems);
    } else {
        echo "Error: SELECT failure in markOrderItemsPaid in
        checkoutProcess.";
        exit(0);
    }
    for ($i = 1; $i <= $numsRecords; $i++) {
        ...
        $success = mysqli_query($db, $query);
        if (!$success) {
            echo"Error:UPDATE failure in markOrderItemsPaid in
            checkoutProcess";
            exit(0);
        }
...
/**
 * Update the number of items in the inverntory
 */
